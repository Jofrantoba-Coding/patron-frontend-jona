...
public class FormVentasProgramada
        extends JDialog
    ...
    private TableModelVentaProgramada modeltable;
    private List<BeanVentaProgramada> listVtaProg;
    private final Logger logger = Logger.getLogger(FormVentasProgramada.class);

    public FormVentasProgramada(Main frame, Usuario usuario, URL ruta, List<BeanVentaProgramada> listVtaProg) {
        super(frame, true);
        this.frmMain = frame;
    ...
    private void loadData() {
        try {
            if (this.listVtaProg == null) {
                RnConsultas logicConsultas = new RnConsultas(path);
                listVtaProg = logicConsultas.listarVentaProgramada(frmMain.getAnio());
            }
            modeltable.clearTable();
            modeltable.agregarListVtaProg(listVtaProg);
    ...
    private void guardar() throws Exception {
        try {
            List<BeanVentaProgramada> listSelected = modeltable.getDataSelected();
            if (listSelected.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Seleccione al menos un documento");
                return;
            }
            RnRegContaCab logic = new RnRegContaCab();
            logic.updateProgramadoRccVenta(listVtaProg);
            for (BeanVentaProgramada beanVtaProg : listVtaProg) {
                this.exportarTxt(beanVtaProg);
            }
            listVtaProg = null;
    ...
    private void exportarTxt(BeanVentaProgramada beanVtaProg) throws Exception {
        try {
            FEtxt feTxt = new FEtxt(path, this.getNameFile(beanVtaProg));
            feTxt.exportarTxt(beanVtaProg.getIdTipoDoc(), beanVtaProg.getSerie(), beanVtaProg.getPreimpreso());
        } catch (Exception e) {
            throw e;
        }
    }

    private String getNameFile(BeanVentaProgramada beanVtaProg) {
