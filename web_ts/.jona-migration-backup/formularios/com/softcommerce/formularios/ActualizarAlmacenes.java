...
public class ActualizarAlmacenes extends CDialog implements ActionListener, KeyListener, WindowListener, ItemListener, FocusListener, MouseListener, PropertyChangeListener {

    private List<UsuarioCorrelativo> x_serie;
    private static final long serialVersionUID = 1L;
    private JTabbedPane tabSearch;
    private ActualizarAlmacenesTableModel modeltable;
    ...
    private JButton btn_cambiaralmacen;
    private JComboBox cbo_idtipodoc;
    private JComboBox cbo_serie;
    private JTextField txt_preimpreso;
    private Usuario usuario;
    private List<Stock> xAlmacenPartida;
    private CComboBox cboAlmacenPartida;
    private JDialog dialog;
    private Gif gif;
    private JFrame frame;

    public ActualizarAlmacenes(JFrame arg0, JHInternal pnlguia, Usuario usuario, java.net.URL path) {
        super(arg0);
        this.path = path;
        this.frame = arg0;
    ...
    public ActualizarAlmacenes(Frame arg0, JHInternalFrame pnlguia, Usuario usuario, java.net.URL path) {
        super(arg0);
        this.path = path;
        this.usuario = usuario;
        initialize();
    ...
    public ActualizarAlmacenes(Frame arg0, Usuario usuario, java.net.URL path) {
        super(arg0);
        this.path = path;
        this.usuario = usuario;
        initialize();
    ...
    public ActualizarAlmacenes(Frame arg0, JHInternalDialog arg1, java.net.URL path) {
        super(arg0);
        this.path = path;
        initialize();
    }
    ...
    private void loadSerie(String id_tipo_doc) {
        try {
            RnCorrelativo regla_PuntoVenta = new RnCorrelativo(path);
            ...
            x_serie.addAll(regla_PuntoVenta.listarUsuarioCorrelativo("", "", "", "", usuario.getCodempresa(), id_tipo_doc, "", "1"));
    ...
    public void cambiarAlmacen() {
        if (tblSearch.getRowCount() == 0 || tblSearch.getSelectedRow() < 0) {
            return;
        }
        ...
        JLabel lblAlmacenPartida = new JLabel("Almacén");
        lblAlmacenPartida.setBounds(10, 20, 60, 20);
        pnl_dialog.add(lblAlmacenPartida);

        cboAlmacenPartida = new CComboBox();
        cboAlmacenPartida.setBounds(65, 20, 240, 20);
        cboAlmacenPartida.addKeyListener(this);
        cboAlmacenPartida.addActionListener(this);
        pnl_dialog.add(cboAlmacenPartida);

        RnStock regla_Almacen = new RnStock(path);

        if (xAlmacenPartida != null) {
            xAlmacenPartida.clear();
        } else {
            xAlmacenPartida = new ArrayList<Stock>();
        }

        Stock a = new Stock();
        a.setIdEmpresa(usuario.getCodempresa());
        a.setIdPuntoVenta(usuario.getCodpuntoventa());
        a.setFechainicio(DateTime.format(1901, 0, 1));
        a.setFechafin(DateTime.format(1901, 0, 1));
        a.setCantFisica(-1);
        a.setIdItem(modeltable.getDocumentoVentaDet(tblSearch.convertRowIndexToModel(tblSearch.getSelectedRow())).getId_item());
        a.setTipoConsulta("2");

        xAlmacenPartida.addAll(regla_Almacen.listarInventarioStock(a));

        cboAlmacenPartida.removeAllItems();

        for (int i = 0; i < xAlmacenPartida.size(); i++) {
            cboAlmacenPartida.addItem(xAlmacenPartida.get(i).getAlmacen());
        }

        RnRegContaCab regla = new RnRegContaCab(path);
        String id_almacen = regla.getIdalmacen(modeltable.getDocumentoVentaDet(tblSearch.convertRowIndexToModel(tblSearch.getSelectedRow())).getId_regcontacab(), modeltable.getDocumentoVentaDet(tblSearch.convertRowIndexToModel(tblSearch.getSelectedRow())).getId_item()).trim();

        if (xAlmacenPartida != null) {
            for (int i = 0; i < xAlmacenPartida.size(); i++) {
                if (xAlmacenPartida.get(i).getIdAlmacen().trim().equals(id_almacen)) {
                    cboAlmacenPartida.setSelectedIndex(i);
                    break;
                }
            }
        }

        JButton btn_cambiar = new JButton("Cambiar Almacen", gif.REFRESH16);
        btn_cambiar.setBounds(100, 55, 150, 20);
        btn_cambiar.setToolTipText("Cambiar");
        btn_cambiar.setContentAreaFilled(true);
        btn_cambiar.setBorderPainted(true);
        btn_cambiar.setFocusable(true);
        btn_cambiar.setFocusPainted(false);
        btn_cambiar.addActionListener(this);
        btn_cambiar.addKeyListener(this);
        btn_cambiar.registerKeyboardAction(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
            }
        }, KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0, false), JComponent.WHEN_FOCUSED);
        btn_cambiar.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                modeltable.getDocumentoVentaDet(tblSearch.convertRowIndexToModel(tblSearch.getSelectedRow())).setId_almacen(xAlmacenPartida.get(cboAlmacenPartida.getSelectedIndex()).getIdAlmacen());
                modeltable.getDocumentoVentaDet(tblSearch.convertRowIndexToModel(tblSearch.getSelectedRow())).setAlmacen(xAlmacenPartida.get(cboAlmacenPartida.getSelectedIndex()).getAlmacen());
                modeltable.fireTableRowsUpdated(tblSearch.convertRowIndexToModel(tblSearch.getSelectedRow()), tblSearch.convertRowIndexToModel(tblSearch.getSelectedRow()));
                dialog.dispose();
            }
        });
        pnl_dialog.add(btn_cambiar);

        dialog = new JDialog(frame);
        dialog.setContentPane(pnl_dialog);
        dialog.setModal(true);
        dialog.setSize(330, 120);
        dialog.setResizable(false);
        dialog.setTitle("Cambiar almacen");
        ComponentToolKit.centerComponentPosicion(dialog);
    ...
    public String executeUpdate() {
        try {
            ...
            con.setMovDet(modeltable.getData());
            con.setIdUsuario(usuario.getId_usuario());
            return regla.modificarAlmacenes(con);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
            return "";
        }
    ...
    public void cargarTabla() {
        ...
        m.setIdTipoDoc((cbo_idtipodoc.getSelectedItem().toString().equals("FACTURA") ? "01" : (cbo_idtipodoc.getSelectedItem().toString().equals("BOLETA") ? "03" : "")));
        m.setIdAuxiliar("00070");
        m.setCantPordespacharCliente(1);

        RnRegContaCab regla = new RnRegContaCab(path);
        modeltable.clearTable();
        modeltable.agregarVectorRegContaItem(regla.listarDetallesOrdenRecojo(m));
        tblSearch.setAllColumnPreferredWidth();
    }
    ...
    public void obtenerDatos() {
        if (!executeUpdate().equals("")) {
            JOptionPane.showMessageDialog(this, "El registro fue grabado correctamente.", "Registros grabados.", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, "No se pudo guardar el registro", "Registros no grabados.", JOptionPane.INFORMATION_MESSAGE);
        }
    }
