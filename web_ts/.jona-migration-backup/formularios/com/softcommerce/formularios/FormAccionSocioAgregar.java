 ...
 */
public class FormAccionSocioAgregar extends JDialog implements ActionListener, FocusListener, KeyListener {
    ...
    private CTable tableSocio;
    private SocioTableModel modeltableSocio;
    private TableRowSorter modeloOrdenadoSocio;
    private JTextField txt_codigo;
    private JTextField txt_numero;
    private JTextField txt_descripcion;
    private Gif gif;

    public FormAccionSocioAgregar(JDialog arg0, java.net.URL path, Component comp, boolean swReg) {
        super(arg0);
    ...
    private boolean verificarValido() {
        if (txtCantidad.getText().length() == 0) {
            ...
            txtCantidad.setBorder(new LineBorder(Color.RED));
        ...
        if (row >= 0) {
            ...
            if (((FormAccionSocio) dialogPadre).modeltableAccionSocio.getRowCount() > 0) {
                for (int i=0;i<((FormAccionSocio) dialogPadre).modeltableAccionSocio.getRowCount();i++){
                    AccionSocio beanAccionSocio=(AccionSocio)((FormAccionSocio) dialogPadre).modeltableAccionSocio.getAccionSocio(i);
                    if (beanSocio.getId_socio()==beanAccionSocio.getBeanSocio().getId_socio() && beanAccionSocio.getEstado().equals("A")){
                        JOptionPane.showMessageDialog(this, "Socio ya esta activado");
                        return false;
                    }
                }
            }
