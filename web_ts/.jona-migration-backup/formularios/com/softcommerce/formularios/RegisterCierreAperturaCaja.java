...
public class RegisterCierreAperturaCaja extends CDialog implements ActionListener, ItemListener, KeyListener, FocusListener, Register {
    ...
    private PanelTFCierreAperturaCaja pnltf;
    private int mode = 0;
    private Gif gif;
    ...
    public RegisterCierreAperturaCaja(Dialog arg0, Usuario usuario) {
        super(arg0);
        this.usuario = usuario;
        inicialize();
    }

    public RegisterCierreAperturaCaja(Frame frame, Usuario usuario, PanelTFCierreAperturaCaja pnltf) {
        super(frame);
        this.usuario = usuario;
        this.pnltf = pnltf;
        inicialize();
    ...
    public boolean isRegisterValid() {
        ...
        if ((txtfechaemision.getText().trim().length() > 0)
            ...
            JOptionPane.showMessageDialog(this, "La fecha de Apertura/Cierre de Caja no es valida. Compruébala e inténtalo de nuevo.", "Datos incompletos de Cierre/Apertura de Caja", 2);
        ...
        if (reg.size() > 0) {
            if (mode == INSERT) {
                JOptionPane.showMessageDialog(this, "La fecha de Apertura/Cierre de Caja ya esta registrada. Compruébala e inténtalo de nuevo.", "Datos incompletos de Cierre/Apertura de Caja", 2);
                txtfechaemision.setBorder(new LineBorder(Color.RED));
                txtfechaemision.requestFocus();
        ...
        if (DateTime.getDateFecha(txtfechaemision.getText()).after(new Date())) {
            JOptionPane.showMessageDialog(this, "No puede aperturar/cerrar una fecha que este mas adelante a este dia. Compruébala e inténtalo de nuevo.", "Datos incompletos de Cierre/Apertura de Caja", 2);
            txtfechaemision.setBorder(new LineBorder(Color.RED));
            txtfechaemision.requestFocus();
