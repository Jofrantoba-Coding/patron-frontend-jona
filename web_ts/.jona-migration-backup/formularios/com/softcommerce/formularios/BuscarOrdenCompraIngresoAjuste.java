...
public class BuscarOrdenCompraIngresoAjuste extends CDialog implements ActionListener, KeyListener, WindowListener, ItemListener, FocusListener, MouseListener, PropertyChangeListener {
    ...
    public BuscarOrdenCompraIngresoAjuste(Main arg0, JHInternal pnlguia, Usuario usuario, java.net.URL path) {
        super(arg0);
        frm = arg0;
        this.path = path;
        this.arg = pnlguia;
    ...
    public boolean seleccionValida() {
        HashSet<String> ordenUnica = new HashSet<String>();
        for (int i = 0; i < tblSearch.getRowCount(); i++) {
            if (modeltable.getDocumentoVentaDet(i).isSeleccionado()) {
                ordenUnica.add(modeltable.getDocumentoVentaDet(i).getTipodocseriepreimpreso());
            }
        }
        if (ordenUnica.size() == 1) {
            return true;
        } else {
            if (ordenUnica.size() > 1) {
                JOptionPane.showMessageDialog(null, "Seleccione Items de una sola Orden de Compra");
            }
            return false;
        }
    }
