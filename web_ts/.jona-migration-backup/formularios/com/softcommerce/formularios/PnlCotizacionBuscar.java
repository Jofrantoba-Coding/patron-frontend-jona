 ...
 */
public class PnlCotizacionBuscar extends JDialog implements ActionListener, MouseListener,KeyListener {
    ...
    public PnlCotizacionBuscar(Frame frame, Usuario usuario, java.net.URL ruta, Object wpnlPadre, Component comp, String strform) {
        super(frame, true);
    ...
    private void cargarTabla() {
        try {
            ...
            List<BeanCotizacionCab> lista = logic.listarCotizacion(new java.sql.Date(fechaInicio.getTime()), new java.sql.Date(fechaFin.getTime()), "P", cboSerie.getSelectedItem().toString(), "");
    ...
    private void obtenerDatos() {
        ...
        if (row >= 0) {
            BeanCotizacionCab bean;
            bean = modeltable.getCotizacion(table.convertRowIndexToModel(row));
            if (pnlPadre != null) {
                if (strform.equals("pedido")) {
                    ((RegisterPedido) pnlPadre).setValueSearch(bean, comp);
                } else {
                    if(com.softcommerce.util.Constans.TIPO_VENTA==3){
                        ((RegisterVentaLote) pnlPadre).setValueSearch(bean, comp);
                    }else if(com.softcommerce.util.Constans.TIPO_VENTA==0){
                        ((RegisterVentaDirecta) pnlPadre).setValueSearch(bean, comp);
                    }else if(com.softcommerce.util.Constans.TIPO_VENTA==1){
                        ((RegisterVentaResumen) pnlPadre).setValueSearch(bean, comp);
                    }else if(com.softcommerce.util.Constans.TIPO_VENTA==2){
                        ((RegisterVentaCosto) pnlPadre).setValueSearch(bean, comp);
                    }                    
                }
                dispose();
            }
    ...
    private void loadSerieCorrelativo() {
        try {
            RnCorrelativo regla_correlativo = new RnCorrelativo(path);
            List<UsuarioCorrelativo> xCorrelativo = new ArrayList<UsuarioCorrelativo>();

            xCorrelativo.addAll(regla_correlativo.listarCorrelativo(usuario.getId_usuario(), usuario.getCodpuntoventa(), "CO"));
