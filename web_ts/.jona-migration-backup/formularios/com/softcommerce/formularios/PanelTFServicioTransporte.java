 ...
 * @author TOSHIBA
 */
public class PanelTFServicioTransporte extends JHInternalFrame implements ListSelectionListener, ControlView, WindowListener, FocusListener, KeyListener, ActionListener, MouseListener, PropertyChangeListener, ItemListener {
    ...
    private RegisterServicioTransporte registeri;
    private JDesktopPane jdp;
    private Usuario usuario;
    private Main frame;

    public PanelTFServicioTransporte(String title, Main frame, JDesktopPane jdp, Usuario usuario) {
    ...
    public PanelTFServicioTransporte(String title, Main frame, JDesktopPane jdp, Usuario usuario, List<Boolean> vPermiso) {
    ...
    public PanelTFServicioTransporte(String title, Main frame, JDesktopPane jdp, Usuario usuario, boolean vendedor) {
    ...
    public PanelTFServicioTransporte(String title, Main frame, JDesktopPane jdp, Usuario usuario, boolean vendedor, boolean supervisor) {
    ...
    public JDialog mostrarFacturacion() {
        dialogo = new JDialog(frame1);
        JLabel lbl_serie = new JLabel("Serie");
        JLabel lbl_preimpreso = new JLabel("Preimpreso");
        JLabel lbl_fecha = new JLabel("Fecha Emision");
        cboSerieFact = new JComboBox();
        txtPreimpresoFact = new JTextField();
        btnGuardarFact = new JButton("Guardar");
        btnGuardarFact = new JButton("Cancelar");
        fechaFact = new JDateChooser("dd/MM/yyyy", "##/##/####", '_');
        dialogo.getContentPane().add(lbl_serie);
        dialogo.getContentPane().add(lbl_preimpreso);
        dialogo.getContentPane().add(lbl_fecha);
        dialogo.getContentPane().add(cboSerieFact);
        dialogo.getContentPane().add(txtPreimpresoFact);
        dialogo.getContentPane().add(fechaFact);
        dialogo.pack();
        return dialogo;
    }
    ...
    public void controlReport(boolean export) {
        ...
        parameters.put(2, "Servicio Transporte");
    ...
    public RowFilter<Object, Object> getFilter() {
        ...
        if (!txt_tmpanexo.getText().equals("")) {
            filters.add(RowFilter.regexFilter(".*" + txt_tmpanexo.getText() + ".*", 2));
        }
        if (!txt_tmpruc.getText().equals("")) {
            filters.add(RowFilter.regexFilter(".*" + txt_tmpruc.getText() + ".*", 3));
        }
        if (!txt_idservicio.getText().equals("")) {
            filters.add(RowFilter.regexFilter(".*" + txt_idservicio.getText() + ".*", 0));
        }
        if (cbo_idalmacen.getSelectedIndex() > 0) {
            filters.add(RowFilter.regexFilter(".*" + cbo_idalmacen.getSelectedItem().toString() + ".*", 8));
        }
    ...
    public void controlPrint(boolean view) {
        ...
        try {
            ...
            reportFast.generarReporte("Ingreso", getSelectedValue(1), "", "", "", "", "", view, false, "Reporte Servicio de Transporte");
        } catch (Exception ex) {
    ...
    public void controlAdd() {
        ...
        if (registeri == null) {
            registeri = new RegisterServicioTransporte(this, "Detalle de Serv.ransporte", usuario, frame);
    ...
    public void controlModify() {
        ...
        if (registeri == null) {
            registeri = new RegisterServicioTransporte(this, "Detalle de Serv.Transporte", usuario, frame);
            registeri.setPath(path);
            registeri.setRowSelection(this);
            registeri.setView(this);
            registeri.setFecha(fechaInicio, fechaFin);
    ...
    public void controlNullify() {
        ...
        if (registeri == null) {
            registeri = new RegisterServicioTransporte(this, "Detalle de Ingreso", usuario, frame);
            registeri.setPath(path);
            registeri.setRowSelection(this);
            registeri.setView(this);
            registeri.setFecha(fechaInicio, fechaFin);
    ...
    public void controlEliminate() {
        ...
        String id_estado = regla_1.getTieneFacturaCompra(modeltable.getIngreso(table.convertRowIndexToModel(table.getSelectedRow())).getIdMovimiento());

        if (!id_estado.trim().equals("N")) {
            JOptionPane.showMessageDialog(frame, "El documento esta ATENDIDO con la FACTURA DE COMPRA N° " + id_estado.trim() + ". "
                    + "\n Para que el documento pueda ser eliminado debe estar en estado Anulado o Pendiente.", "Eliminar Documento", JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        String tienefactura = regla_1.getTieneFacturaCompra(modeltable.getIngreso(table.convertRowIndexToModel(table.getSelectedRow())).getIdMovimiento());

        if (!tienefactura.trim().equals("N")) {
            JOptionPane.showMessageDialog(frame, "El documento esta ATENDIDO con la FACTURA DE FLETE N° " + id_estado.trim() + ". "
                    + "\n Para que el documento pueda ser eliminado debe estar en estado Anulado o Pendiente.", "Eliminar Documento", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
    ...
    public void cargarTabla() {
        ...
        try {
            modeltable.agregarVectoringreso(regla.listarServicio("", "", fechaInicio, fechaFin));
//            modeloOrdenado.setRowFilter(getFilter());
//            table.setAllColumnPreferredWidth();
        } catch (Exception e) {
