 ...
 * @author TOSHIBA
 */
public class PanelTFTrabajador extends JHInternalFrame implements ListSelectionListener, WindowListener, FocusListener, KeyListener, ActionListener, ItemListener {
    ...
    public PanelTFTrabajador(String title, JFrame frame, Usuario usuario) {
        super(title);
    ...
    public PanelTFTrabajador(String title, JFrame frame, Usuario usuario, List<Boolean> vPermiso) {
    ...
    public RowFilter getFilter() {
        ...
        if (txt_idtrabajador.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_idtrabajador.getText().trim() + ".*", 0));
        }

        if (txt_nombretrabajador.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_nombretrabajador.getText().trim() + ".*", 1));
        }

        if (txt_dniruc.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_dniruc.getText().trim() + ".*", 2));
        }

        if (cboTipoTrabajador.getSelectedIndex() > 0) {
            filters.add(RowFilter.regexFilter(".*" + xTipoTrabajador.get(cboTipoTrabajador.getSelectedIndex() - 1).getId_tipo_trabajador() + ".*", 3));
        }
    ...
    public void loadTipoTrabajador() {
        try {
            ...
            if (xTipoTrabajador != null) {
                ...
                xTipoTrabajador = new ArrayList();
            }
            xTipoTrabajador.addAll(logic.listarTipoTrabajador("", "A"));

            cboTipoTrabajador.removeAllItems();
            cboTipoTrabajador.addItem("---TODOS---");
    ...
    public void controlAdd() {
        ...
        RegisterTrabajador register = new RegisterTrabajador(frame, usuario);
    ...
    public void cargarTabla() {
        try {
            ...
            modeltable.agregarListTrabajador(regla.listarTrabajador("", "", ""));
            modeloOrdenado.setRowFilter(getFilter());
            table.setAllColumnPreferredWidth();
        } catch (Exception ex) {
