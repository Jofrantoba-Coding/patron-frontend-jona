...
public class RegisterNotaSalida
        extends JHInternal
        implements ActionListener, ItemListener, KeyListener, FocusListener, TableModelListener, MouseListener {
    ...
    private List<Localidad> lst_localidad;
    private List<PuntoVenta> lst_puntoventa;
    private List<Almacen> lst_almacen;
    private List<UsuarioCorrelativo> lst_serie;
    private List<BeanMoneda> lst_moneda;
    ...
    private JTextField txt_idordencompra;
    ...
    private JTextField txtNumeroTransportista;
    private JTabbedPane tabb_notaSalida;
    private JCheckBox chkSeleccionar;
    private JTable tblNotaSalida;
    ...
    public RegisterNotaSalida(String title, Usuario usuario, Main frm) {
        super(title);
    ...
    public void loadLocalidad() {
        try {
            ...
            if (lst_localidad != null) {
                lst_localidad.clear();
            } else {
                lst_localidad = new ArrayList<Localidad>();
            }

            lst_localidad.addAll(regla_Localidad.listar("", usuario.getCodempresa(), "", "", ""));

            cbo_idlocalidad.removeAllItems();
            cbo_idlocalidad.addItem("--- Seleccione una Localidad ---");

            for (int i = 0; i < lst_localidad.size(); i++) {
                cbo_idlocalidad.addItem(lst_localidad.get(i).getDescripcion());
            }
    ...
    public void loadPuntoVenta(String xcodLocalidad) {
        try {
            ...
            if (lst_puntoventa != null) {
                lst_puntoventa.clear();
            } else {
                lst_puntoventa = new ArrayList<PuntoVenta>();
            }

            lst_puntoventa.addAll(regla_PuntoVenta.listar("", "", xcodLocalidad, "", "", "", "", ""));

            cbo_idpuntoventa.removeAllItems();
            cbo_idpuntoventa.addItem("--- Seleccione un Punto de Venta ---");

            for (int i = 0; i < lst_puntoventa.size(); i++) {
                cbo_idpuntoventa.addItem(lst_puntoventa.get(i).getDescripcion_puntoventa());
            }
    ...
    public void loadAlmacen(String xcodPuntoventa) {
        try {
            ...
            if (lst_almacen != null) {
                ...
                lst_almacen = new ArrayList<Almacen>();
            }

            lst_almacen.addAll(regla_Almacen.listar("", "", xcodPuntoventa));

            cbo_idalmacen.removeAllItems();
            cbo_idalmacen.addItem("--- Seleccione un Almacen ---");

            for (int i = 0; i < lst_almacen.size(); i++) {
                cbo_idalmacen.addItem(lst_almacen.get(i).getDescripcion());
            }
    ...
    public void mostrarPreimpreso() {
        try {
            ...
            String preimpreso = regla_correlativo.listarPreimpreso(lst_serie.get(cbo_serie.getSelectedIndex()).getIdCorrelativo());
    ...
    public void loadSerie() {
        try {
            ...
            if (lst_serie != null) {
                ...
                lst_serie = new ArrayList<UsuarioCorrelativo>();
            }
            lst_serie.addAll(regla_correlativo.listarCorrelativo(usuario.getId_usuario(), usuario.getCodpuntoventa(), "OS"));

            cbo_serie.removeAllItems();

            for (int i = 0; i < lst_serie.size(); i++) {
                cbo_serie.addItem(lst_serie.get(i).getSerie());
            }
    ...
    public void loadMoneda() {
        try {
            lst_moneda = new ArrayList();
            LoadCombo.loadMoneda(path, lst_moneda, cbo_idmoneda,
    ...
    public void newRegister() {
        ...
        dc_fechaemision.setDate(new Date(Main.fechaActualServer.getTime()));
    }
    ...
    public void cargarLocalidad(String xcodiLocalidad) {
        if (lst_localidad != null && !xcodiLocalidad.equals("")) {
            for (int i = 0; i < lst_localidad.size(); i++) {
                if (lst_localidad.get(i).getId_localidad().equals(xcodiLocalidad)) {
    ...
    public void cargarPuntoVenta(String codPuntoVenta) {
        if (lst_puntoventa != null && !codPuntoVenta.equals("")) {
            for (int i = 0; i < lst_puntoventa.size(); i++) {
                if (lst_puntoventa.get(i).getId_punto_venta().equals(codPuntoVenta)) {
    ...
    public void cargarAlmacen(String xcodiEmpresa) {
        if (lst_almacen != null && !xcodiEmpresa.equals("")) {
            for (int i = 0; i < lst_almacen.size(); i++) {
                if (lst_almacen.get(i).getIdAlmacen().equals(xcodiEmpresa)) {
    ...
    public void cargarMoneda(String xcodiEmpresa) {
        if (lst_moneda != null && !xcodiEmpresa.equals("")) {
            for (int i = 0; i < lst_moneda.size(); i++) {
                if (lst_moneda.get(i).getIdMoneda().equals(xcodiEmpresa)) {
    ...
    public void cargarDocumentoVenta(List<ContaItem> m) {
        ContaCab contaCab = m.get(0).getContacab();
        txt_razonsocial.setText(contaCab.getAnTmpanexo());
        txt_ruc.setText(contaCab.getAnTmpruc());
        txt_idordencompra.setText(contaCab.getRcIdregconta());
        txtIdTipoDocRef.setText("OC");
        txt_serieref.setText(contaCab.getSerie());
        txt_preimpresoref.setText(contaCab.getPreimpreso());
        txt_codcliente.setText(contaCab.getAnIdanexo());
        cargarAlmacen(contaCab.getIdAlmacen());
        cargarMoneda(contaCab.getIdMoneda());
        txt_tipocambio.setText(String.valueOf(contaCab.getMTipoCambio()));
        txt_afecto.setText(String.valueOf(contaCab.getMAfecto()));
        txt_noafecto.setText(String.valueOf(contaCab.getMNoafecto()));
        txt_monto.setText(String.valueOf(contaCab.getMonto()));
        txt_total.setText(String.valueOf(contaCab.getTotal()));
        txt_igv.setText(String.valueOf(contaCab.getMIgv()));
        //txtIdTransportista.setText(contaCab.getId_anexo_transportista());
        this.loadDataTransportista(contaCab.getIdAnexoTransportista());
        txt_flagdescuento.setText(contaCab.getFlagDescuento());
        txt_flagdetigv.setText(contaCab.getFlagigv());

        mdlNotaSalida.clearTable();
        chkSeleccionar.setSelected(true);
        if (Constans.ISBOTICA && !Constans.IS_NOTA_SALIDA_AUTOMATICO) {
            for (ContaItem bean : m) {
                bean.setCantidad_despachar("0");
            }
        }
    ...
    private void loadDataTransportista(String idTransportista) {
        try {
            txtIdTransportista.setText(idTransportista);
            if (idTransportista == null) {
                return;
            }
            if (idTransportista.isEmpty()) {
                return;
            }
            RnAnexo rnAnexo = new RnAnexo(path);
            BeanAnexo beanAnexo = rnAnexo.beanAnexoImp(TipoAnexoEnum.TRANSPORTISTA.getValue(), idTransportista, "", "", "");
            if (beanAnexo != null) {
                txtTransportista.setText(beanAnexo.getDescripcion());
                txtNumeroTransportista.setText(beanAnexo.getNumero());
            } else {
                txtTransportista.setText("");
                txtNumeroTransportista.setText("");
            }
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, e.getMessage());
    ...
    private void setLoteDetalles() throws Exception {
        try {
    ...
    private ArrayList<Lote> getLotesInsertar(ContaItem contaItem) throws Exception {
        try {
    ...
    private List<Lote> getLotes(ContaItem contaItem) throws Exception {
        try {
            LogicLote logicLote = new LogicLote(Main.path);
            List<Lote> lotes = logicLote.lotesConversion(contaItem.getId_item(), contaItem.getId_almacen());
            return lotes;
        } catch (Exception e) {
            throw e;
        }
    ...
    private ContaCab getContaCab() throws Exception {
        try {
    ...
    public String executeInsert() {
        try {
            ...
            return logic.insertarSalidaAjusteInventario(this.getContaCab());
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean isRegisterValid() {
        return this.validateBotica();
    }

    private boolean validateBotica() {
        if (!Constans.ISBOTICA) {
            return true;
        }
        if (!Constans.IS_NOTA_SALIDA_AUTOMATICO){
    ...
    private boolean validateItem() {
        for (int i = 0; i < mdlNotaSalida.getRowCount(); i++) {
            ContaItem contaItem = mdlNotaSalida.getDocumentoVentaDet(i);
            if (!validateItem(contaItem)) {
                JOptionPane.showMessageDialog(this, "Item : " + contaItem.getId_item() + ", no cuenta con saldo suficiente");
                return false;
            }
        }
        return true;
    ...
    private boolean validateItem(ContaItem contaItem) {
    ...
    private BigDecimal getStockByItemAlmacen(ContaItem contaItem) {
        try {
            RnStock rnStock = new RnStock(path);
            return rnStock.getStockByItemAlmacen(contaItem.getId_item(), contaItem.getId_almacen());
        } catch (Exception e) {
            e.printStackTrace();
