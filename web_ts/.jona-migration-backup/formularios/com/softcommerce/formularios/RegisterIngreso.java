...
public class RegisterIngreso
        extends JHInternal
        implements ActionListener, ItemListener, KeyListener, FocusListener, TableModelListener {
    ...
    public JLabel lblSerie = new JLabel("N° INGRESO");
    ...
    private final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(RegisterIngreso.class);
    private FormConversionVarios pnlConversionVarios;

    public RegisterIngreso(PanelTFIngreso pnltf, String title, Usuario usuario, Main frm) {
        super(title);
    ...
    private void inicialize() {
        ...
        lblNumdocreferencia = new JLabel("N° DOC");
            ...
            while (iterador.hasNext()) {
                ...
                if (bean.isSeleccionado() && bean.getCant_a_devolver() != null && !bean.getCant_a_devolver().isEmpty() && Double.parseDouble(bean.getCant_a_devolver()) > 0) {
                    ...
                    if (com.softcommerce.util.Constans.ISBOTICA && com.softcommerce.util.Constans.ES_OBLIGATORIO_UBICACION) {
                        if (!validLotes(bean)) {
                            throw new Exception("Seleccion la ubicación de sus lotes\ny edite las cantidades si es necesario");
                        }
                    }
                }
            }
    ...
    public boolean isRegisterValid() {
        ...
        if (txtPreimpresoNumDocReferencia.getText().trim().length() < 10 && txtPreimpresoNumDocReferencia.isVisible()) {
            JOptionPane.showMessageDialog(this, "El numero de Preimpreso de la Orden de Compra de Referencia que has especificado no es válida. Compruébala e inténtalo de nuevo.", "Datos incompletos de Nota de Ingreso.", JOptionPane.INFORMATION_MESSAGE);
        ...
        if (txtPreimpresoGuiaReferencia.getText().trim().length() < 10 && txtPreimpresoGuiaReferencia.isVisible()) {
            JOptionPane.showMessageDialog(this, "El numero de Preimpreso de Guia de Remision de Referencia que has especificado no es válida. Compruébala e inténtalo de nuevo.",
        ...
        if (((JTextFieldDateEditor) dcFechaEmision.getDateEditor()).getText().equals("__/__/____")
            ...
            JOptionPane.showMessageDialog(this, "La fecha de Emision de la Nota de Ingreso que has especificado no es válida. Compruébala e inténtalo de nuevo.", "Datos incompletos de Nota de Ingreso", 2);
    ...
    public void cargarFacturaTransito(
        ...
        try {
            ...
            Logger.getLogger(RegisterIngreso.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, ex.getMessage());
        } catch (InstantiationException ex) {
            Logger.getLogger(RegisterIngreso.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, ex.getMessage());
