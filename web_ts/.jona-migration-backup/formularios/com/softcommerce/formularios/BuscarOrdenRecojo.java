...
public class BuscarOrdenRecojo
        extends CDialog 
        implements ActionListener, KeyListener, WindowListener, ItemListener, FocusListener {
    ...
    public BuscarOrdenRecojo(Frame arg0, JHInternal pnlguia, Usuario usuario, URL path) {
        super(arg0);
        this.path = path;
        this.arg = pnlguia;
    ...
    private void initialize() {
        ...
        setTitle("Buscar Documento de Venta - BuscarOrdenRecojo");
        setContentPane(pnl);
        setModal(true);
        setResizable(true);
        setSize(880, 440);
    ...
    private JPanel getPnlBuscarPor() {
    ...
    private void buscarDocumento() {
        if (this.validateBusqueda()) {
            ...
            cuadro.CuadroAviso("Para buscar todos los items pendientes de salida, debes especificar \nel tipo de documento y su numero de preimpreso"
                    + " indicar el cliente.", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    public void cargarTabla() {
        try {
            ...
            r.setIdTipoDoc(this.getIdTipoDoc());
            ...
            List<ContaItem> lista = regla.listarDetallesOrdenRecojo(r);
            if (lista.size() > 0) {
                modeltable.agregarVectorRegContaItem(lista);
            } else {
