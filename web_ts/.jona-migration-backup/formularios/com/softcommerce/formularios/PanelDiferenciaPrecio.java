...
public class PanelDiferenciaPrecio
        extends CDialog implements ActionListener, KeyListener, WindowListener, FocusListener {

    private PnlDiferenciaPrecioTableModel mdl_documento;
    private CTable tbl_documento;
    private JScrollPane scp_documento;
    ...
    public PanelDiferenciaPrecio(Frame arg0, JHInternal arg, Usuario usuario, java.net.URL path) {
        super(arg0);
        this.frame = arg0;
        this.path = path;
        this.arg = arg;
    ...
    public void calcularImportesDescuento() {
        ...
        BigDecimal m_descuento_total = BigDecimal.ZERO;
        ContaItem obtab;
        for (int i = 0; i < mdl_documento.getRowCount(); i++) {

            if (Boolean.parseBoolean(mdl_documento.getValueAt(i, 0).toString())) {
                obtab = mdl_documento.getVentaDirecta(i);
                monto = monto.add(BigDecimal.valueOf(obtab.getM_monto_act()));
                m_afecto = m_afecto.add(BigDecimal.valueOf(obtab.getM_afecto_act()));
                m_noafecto = m_noafecto.add(BigDecimal.valueOf(obtab.getM_noafecto_act()));
                m_igv = m_igv.add(BigDecimal.valueOf(obtab.getM_igv_act()));
                m_descuento = m_descuento.add(BigDecimal.valueOf(obtab.getM_descuento()));
                m_descuento_total = m_descuento_total.add(BigDecimal.valueOf(obtab.getM_descuento_total()));
            }
        }

        txt_descuento.setText(m_descuento.toString());
        txt_afecto.setText(m_descuento_total.divide(new BigDecimal("1.18"), 2, RoundingMode.CEILING).toString());
        txt_igv.setText(new BigDecimal(txt_afecto.getText().trim()).multiply(new BigDecimal(0.18)).setScale(2, RoundingMode.CEILING).toString());
        txt_noafecto.setText(m_noafecto.compareTo(new BigDecimal(0)) > 0 ? m_descuento_total.subtract(new BigDecimal(txt_igv.getText())).setScale(2, RoundingMode.HALF_DOWN).toString() : "0");

        txt_monto.setText(m_descuento_total.setScale(2, RoundingMode.HALF_DOWN).toString());
        txt_percepcion.setText(m_percepcion.setScale(2, RoundingMode.HALF_DOWN).toString());
        BigDecimal montoTotal = monto.add(m_percepcion);
        txt_total.setText(m_descuento_total.setScale(2, RoundingMode.HALF_DOWN).toString());
    }
    ...
    public void limpiar() {
        ...
        dc_fechainicio.setDate(new Date(100, 0, 1));
    ...
    public void cargarTabla() {
        ...
        m.setAnIdanexo(txt_idanexo.getText().trim());
        m.setSerie(txt_serie.getText().trim());
        m.setPreimpreso(txt_preimpreso.getText().trim());
        m.setIdTipoDoc(cbo_idtipodoc.getSelectedItem().equals("T") ? "" : (cbo_idtipodoc.getSelectedItem().equals("F") ? "01" : (cbo_idtipodoc.getSelectedItem().equals("B") ? "03" : "")));
        m.setIdEstado(cbo_idestado.getSelectedItem().equals("T") ? "" : (cbo_idestado.getSelectedItem().equals("PEN") ? "003" : (cbo_idestado.getSelectedItem().equals("PAA") ? "004" : (cbo_idestado.getSelectedItem().equals("ATE") ? "005" : ""))));

        mdl_documento.clearTable();
        List<ContaItem> lista = new ArrayList();
        mdl_documento.agregarVectorVentaDirecta(lista);
        tbl_documento.setAllColumnPreferredWidth();
    }
    ...
    private void cargarDocumentoReferenciaDocumentoCambioPrecio(ContaCab r) {
        try {
            ...
            if (reg != null && reg.size() > 0) {
                ...
                v.addAll(regla.listarDetalleNotaCreditoDiferenciaPrecio(r));
                ContaItem obj;
                ...
                mdl_documento.clearTable();
                mdl_documento.agregarVectorVentaDirecta(v);
                tbl_documento.setAllColumnPreferredWidth();
                tbl_documento.getColumnModel().getColumn(0).setPreferredWidth(30);
                tbl_documento.getColumnModel().getColumn(1).setPreferredWidth(65);
            }
        } catch (Exception ex) {
        }
    }
    ...
    public void setValueSearch(Object valor, Component comp) {
        ...
        cargarDocumentoReferenciaDocumentoCambioPrecio(r);
    }
    ...
    public void obtenerDatos() {
        List<ContaItem> lista2 = mdl_documento.getData();
        List<ContaItem> lista = new ArrayList();
        ContaItem obj;
        ContaCab cab = lista2.get(0).getContacab();
        for (int i = 0; i < lista2.size(); i++) {
            obj = lista2.get(i);
            if (obj.isSeleccionado()) {
                if (obj.getP_unit_act() <= 0) {
                    JOptionPane.showMessageDialog(null, "Debe colocar un precio mayor a 0");
                    return;
                }
            }
        }
        ...
        for (int i = 0; i < lista2.size(); i++) {
            ...
            obj.setContacab(cab);
            if (obj.isSeleccionado()) {
                obj.setP_unit(obj.getP_unit_act());
                obj.setM_descuento(obj.getM_descuento());
                obj.setM_descuento_total(obj.getM_descuento() * Double.parseDouble(obj.getCant_string()));
                obj.setM_afecto(obj.getM_descuento_total() / 1.18);
                obj.setM_igv(obj.getM_afecto() * 0.18);
                obj.setM_noafecto((obj.getM_noafecto_act() > 0 ? obj.getM_descuento_total() - obj.getM_igv() : 0));

                obj.setM_valor(obj.getM_descuento_total());
                obj.setM_monto(obj.getM_descuento_total());
                obj.setM_total(obj.getM_descuento_total());
                lista.add(obj);
