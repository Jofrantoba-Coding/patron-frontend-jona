 ...
 */
public class PanelTFMotivoDescuento extends JHInternalFrame implements ListSelectionListener, WindowListener, FocusListener, KeyListener, ActionListener, ItemListener {
    ...
    private JTextField txt_idMotiDescuento;
    private JTextField txt_descripcionMotivoDescuento;

    public PanelTFMotivoDescuento(String title, JFrame frame, Usuario usuario) {
        super(title, true, true, true,false, false, true, true, false, false, false, false, true, true);
        this.usuario = usuario;
        this.frame = frame;
        inicialize();
    }
    public PanelTFMotivoDescuento(String title, JFrame frame, Usuario usuario, List<Boolean> vPermiso) {
    ...
    public void cargarTabla() {
        try {
            ...
            modeltable.agregarVectorMotivoDescuento(regla.listar(0));
            modeloOrdenado.setRowFilter(getFilter());
            table.setAllColumnPreferredWidth();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this,"Error: " + e.getMessage(),"Motivo Dscto",JOptionPane.OK_OPTION);
        }
    }
    ...
    public RowFilter getFilter() {
        ...
        if (txt_idMotiDescuento.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_idMotiDescuento.getText().trim() + ".*", 1));
        }

        if (txt_descripcionMotivoDescuento.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_descripcionMotivoDescuento.getText().trim() + ".*", 2));
        }
    ...
    public void controlAdd() {
        try {
            ...
            RegisterMotivoDescuento register = new RegisterMotivoDescuento(frame, usuario);
    ...
    public void controlReport(boolean export) {
        ...
        parameters.put(2, "Motivo Descuento");
