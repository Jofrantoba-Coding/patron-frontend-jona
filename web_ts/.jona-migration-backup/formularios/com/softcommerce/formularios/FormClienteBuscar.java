 ...
 */
public class FormClienteBuscar extends JDialog implements ActionListener, KeyListener, WindowListener, ItemListener, FocusListener, MouseListener {
    ...
    public FormClienteBuscar(JHDialog arg0, java.net.URL path, Component comp) {
        super(arg0);
        this.dialog = arg0;
        this.comp = comp;
        this.path = path;
    ...
    public void cargarDatos() {
        try {
            ...
            modeltableCliente.agregarVectorCliente(regla.listarCliente("A"));
    ...
    private RowFilter getFilter() {
        ...
        if (txtNumero.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txtNumero.getText().trim() + ".*", 9));
        }
    ...
    private void obtenerDatos() {
        ...
        if (row >= 0) {
            BeanCliente beanCliente = modeltableCliente.getCliente((Integer) tableCliente.getValueAt(row, 0) - 1);
            if (((RegisterClienteNuevo) dialog).modeltableClienteSustituto.getRowCount() > 0) {
                for (int i = 0; i < ((RegisterClienteNuevo) dialog).modeltableClienteSustituto.getRowCount(); i++) {
                    ClienteSustituto beanClienteSust = ((RegisterClienteNuevo) dialog).modeltableClienteSustituto.getClienteSustituto(i);
                    if (beanClienteSust.getBeanCliente().getIdCliente().equals(beanCliente.getIdCliente())) {
                        JOptionPane.showMessageDialog(this, "Cliente ya Esta Agregado");
                        return;
                    }
                }
            }
            //Verificar si ya esta Agregador
            ClienteSustituto beanClienteSustituto = new ClienteSustituto();
            beanClienteSustituto.setBeanCliente(beanCliente);
            beanClienteSustituto.setEstado("A");
            beanClienteSustituto.setOperacion("I");
            dialog.setValueSearch(beanClienteSustituto, comp);
            dispose();
        } else {
