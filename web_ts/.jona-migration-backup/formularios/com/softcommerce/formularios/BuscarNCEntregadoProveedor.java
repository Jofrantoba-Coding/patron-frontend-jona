...
public class BuscarNCEntregadoProveedor extends CDialog implements ActionListener, KeyListener, WindowListener, ItemListener, FocusListener, MouseListener, PropertyChangeListener {

    private JTabbedPane tabb_documento;
    private BuscarDocumentoTableModel mdl_documento;
    private SolicitudDevolucionTableModel mdl_solicitud;
    ...
    public BuscarNCEntregadoProveedor(Frame arg0, JHInternal arg, Usuario usuario, java.net.URL path) {
        super(arg0);
        this.path = path;
        this.arg = arg;
...
//        pnl.add(txt_tmpruc);
        ...
        setTitle("Buscar Documentos de Venta con Entrega de Productos Realizado - BuscarNCEntregadoProveedor");
    ...
    public void cargarTabla() {
        try {
            //tania
            SolicitudDevolucion bean = new SolicitudDevolucion();
            bean.setPuntoVenta(usuario.getCodpuntoventa());
            LogicSolicitudDev logica = new LogicSolicitudDev(path);
            mdl_documento.clearTable();
            mdl_solicitud.clearTable();

            String tipo_doc = "%";
//tania
            List<SolicitudDevolucion> listaDev = null;
            if (this.tipo.equals("005")) {
                listaDev = logica.listarSolicitud(usuario.getCodpuntoventa(),
                        txt_preimpreso.getText().trim(), txt_serie.getText().trim(), dc_fechainicio.getDate(),
                        dc_fechafin.getDate(), txt_idanexo.getText().trim());
            } else if (this.tipo.equals("016")) {
                listaDev = logica.listarSolicitudPaq(usuario.getCodpuntoventa(),
                        txt_preimpreso.getText().trim(), txt_serie.getText().trim(), dc_fechainicio.getDate(),
                        dc_fechafin.getDate(), txt_idanexo.getText().trim());
            }
            if (listaDev.size() > 0) {
                mdl_solicitud.agregarVectorSolicitudDev(listaDev);
            } else {
