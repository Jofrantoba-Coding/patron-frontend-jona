...
public class RegisterNotaDebito
        extends JHInternal
        implements KeyListener, ActionListener, FocusListener, ItemListener, TableModelListener {
    private final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(RegisterNotaDebito.class);
    ...
    public RegisterNotaDebito(PanelTFNotaDebito pnltfnotacredito, String title, Usuario usuario, JFrame frame) {
        super(title);
                ...
                try {
                    if (txt_idcondicionventa.getSelectedItem().toString().equalsIgnoreCase("CR")) {
                        if (dc_fechafin.getDate().compareTo(dc_fechainicio.getDate()) == -1) {
                            JOptionPane.showMessageDialog(null, "Fecha vencimiento menor a fecha de emisión");
                            dc_fechafin.setDate(dc_fechainicio.getDate());
                        }
                    }
                } catch (Exception ex) {
    ...
    private void exportarTxt(ContaCab beanRcc) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "Comprobante electrónico fue generada");
        } catch (Exception e) {
            ...
            if (x_idmovimiento.get(cbo_idmovimiento.getSelectedIndex()).getId_motivo().equals("012")) {
                ...
                if (txt_idcondicionventa.getSelectedItem().toString().trim().equals("CO")) {
                    ...
                    try {
                        ...
                        Logger.getLogger(RegisterNotaDebito.class.getName()).log(Level.SEVERE, null, ex);
                        JOptionPane.showMessageDialog(null, ex.getMessage());
                        return null;
                    }
                } else {
