// SkeletonPresets.tsx — Level 2: Molecule
// Composed skeleton loading states. Inspired by shadcn/ui Skeleton examples.
// Combines SkeletonAtom into meaningful loading placeholders.
...
/** Card with header + content lines */
export const SkeletonCard: React.FC = () => (
  <div className="rounded-token-lg border border-neutral-200 p-6 space-y-4">
    ...
    <div className="flex gap-2 pt-2">
      <SkeletonAtom className="h-8 w-20 rounded-token-md" />
      <SkeletonAtom className="h-8 w-20 rounded-token-md" />
    </div>
  </div>
);
    ...
    {Array.from({ length: rows }).map((_, r) => (
      <div key={r} className="flex gap-4">
        {Array.from({ length: cols }).map((_, c) => (
          <SkeletonAtom key={c} className="h-4 flex-1" />
        ))}
      </div>
    ))}
  </div>
  ...
  <div className="space-y-4">
    {Array.from({ length: fields }).map((_, i) => (
      <div key={i} className="space-y-1.5">
        ...
        <SkeletonAtom className="h-9 w-full rounded-token-md" />
      </div>
    ))}
    <SkeletonAtom className="h-9 w-28 rounded-token-md" />
  </div>
);
