 ...
 */
package com.softcommerce.views.uipnlpedidobuscar;
 ...
 */
public class UiPnlPedidoBuscar extends JDialog implements InterUiPnlPedidoBuscar, ActionListener, MouseListener {
    ...
    public UiPnlPedidoBuscar(Frame frame, Usuario usuario, java.net.URL ruta, Object wpnlPadre, Component comp, String strform) {
        super(frame, true);
    ...
    private void cargarTabla() {
        try {
            ...
            List<BeanPedidoCab> lista = logic.listarPedido(new java.sql.Date(fechaInicio.getTime()), new java.sql.Date(fechaFin.getTime()), "P", cboSerie.getSelectedItem().toString(),"");
    ...
    private void obtenerDatos() {
        ...
        if (row >= 0) {
            BeanPedidoCab bean = new BeanPedidoCab();
            bean = modeltable.getPedido(table.convertRowIndexToModel(row));
            if (pnlPadre != null) {
    ...
    private void loadSerieCorrelativo() {
        try {
            ...
            xCorrelativo.addAll(regla_correlativo.listarCorrelativo(usuario.getId_usuario(), usuario.getCodpuntoventa(), "PD"));
