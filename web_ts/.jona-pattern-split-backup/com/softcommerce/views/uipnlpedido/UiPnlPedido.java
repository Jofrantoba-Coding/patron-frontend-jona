 ...
 */
public class UiPnlPedido extends JHInternalFrame implements InterUiPnlPedido, ListSelectionListener, WindowListener, FocusListener, KeyListener, ActionListener, ItemListener, MouseListener, PropertyChangeListener {

    private CTable table;
    private PedidoTableModel modeltable;
    ...
    public UiPnlPedido(String title, JFrame frame, Usuario usuario) {
    ...
    public UiPnlPedido(String title, JFrame frame, Usuario usuario, List<Boolean> vPermiso) {
    ...
    private void inicialize() {
        ...
        super.getPnlControl().getCbClone().setText("Cerrar Pedido");
        modeltable = new PedidoTableModel();
    ...
    public void cargarTabla() {
        try {
            ...
            List<BeanPedidoCab> lista = logic.listarPedido(new java.sql.Date(fechaInicio.getTime()), new java.sql.Date(fechaFin.getTime()), "", "", "");
            modeltable.agregarListPedido(lista);
    ...
    public void controlModify() {
        ...
        if (!(beanPedido.getBeanEstadoDocumento().getIdEstado().equals("003") || beanPedido.getBeanEstadoDocumento().getIdEstado().equals("004"))) {
    ...
    public void controlEliminate() {
        ...
        int realRowIndex = table.convertRowIndexToModel(visibleRowIndex);
        BeanPedidoCab beanPedido = modeltable.getPedido(realRowIndex);
        if (beanPedido.getBeanEstadoDocumento().getIdEstado().equals("004") || beanPedido.getBeanEstadoDocumento().getIdEstado().equals("005") || beanPedido.getBeanEstadoDocumento().getIdEstado().equals("008")) {
    ...
    public void controlNullify() {
        ...
        int realRowIndex = table.convertRowIndexToModel(visibleRowIndex);
        BeanPedidoCab beanPedido = modeltable.getPedido(realRowIndex);
        if (beanPedido.getBeanEstadoDocumento().getIdEstado().equals("001")) {
            JOptionPane.showMessageDialog(this, "DOCUMENTO YA SE ENCUENTRA ANULADO");
            return;
        }
        if (!beanPedido.getBeanEstadoDocumento().getIdEstado().equals("003")) {
    ...
    public void controlPrint(boolean view) {
        try {
            ...
            String montoLetras = conv.convertir(Double.parseDouble(modeltable.getPedido(realRowIndex).getMonto().toString()), modeltable.getPedido(realRowIndex).getBeanMoneda().getIdMoneda());
            ...
            listaRpt = logic.reportCotizacion(modeltable.getPedido(realRowIndex).getId_cotizacion());
            rutaJasper = Constans.PATH_RPT_JASPER + "rptCotizacionNvo.jasper";
