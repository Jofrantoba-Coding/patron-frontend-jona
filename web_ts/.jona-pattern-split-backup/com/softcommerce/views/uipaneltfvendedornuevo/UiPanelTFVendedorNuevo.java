 ...
 * @author TOSHIBA
 */
public class UiPanelTFVendedorNuevo extends JHInternalFrame implements InterUiPanelTFVendedorNuevo, View, ListSelectionListener, RowSelection, ControlView, WindowListener, FocusListener, KeyListener, ActionListener {

    public CTable table;
    public VendedorTableModel modeltable;
    ...
    private JTextField txt_numlicencia;
    ...
    public UiPanelTFVendedorNuevo(String title, JFrame frame, Usuario usuario) {
        super(title);
    ...
    public UiPanelTFVendedorNuevo(String title, JFrame frame, Usuario usuario, List<Boolean> vPermiso) {
    ...
    private void inicialize() {
        modeltable = new VendedorTableModel();
    ...
    public void controlReport(boolean export) {
        ...
        parameters.put(2, "Vendedor Nuevo");
    ...
    public RowFilter getFilter() {
        ...
        if (txt_descripcionanexo.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_descripcionanexo.getText().trim() + ".*", 2));
        }
        ...
        if (txt_tmpruc.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_tmpruc.getText().trim() + ".*", 3));
        }
    ...
    public void controlAdd() {
        ...
        RegisterVendedor register = new RegisterVendedor(frame, usuario);
...
//        a.setId_empresa(usuario.getCodempresa());
        ...
        try {
            modeltable.agregarListaVendedor(regla.listarVendedor(vend));
            modeloOrdenado.setRowFilter(getFilter());

            CellRendererImageEstado obj = new CellRendererImageEstado();
            DefaultTableCellRenderer renderer = new javax.swing.table.DefaultTableCellRenderer();
            DefaultTableCellRenderer renderer2 = new javax.swing.table.DefaultTableCellRenderer();
            Imagenes img = new Imagenes();
            renderer.setIcon(img.getIconActivado());
            renderer2.setIcon(img.getIconDesactivado());
            //Object g = table.getModel().getValueAt(0, 34);
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error: " + e.getMessage(), "Vendedor", JOptionPane.OK_OPTION);
        }
        ...
        this.table.getColumnModel().getColumn(7).setCellRenderer(new CellRender());
        this.table.getColumnModel().getColumn(8).setCellRenderer(new CellRender());
