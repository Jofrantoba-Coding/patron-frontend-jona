...
public class UiFacturarServicioTransporte 
        extends JHInternal 
        implements InterUiFacturarServicioTransporte, ActionListener, ItemListener, KeyListener, FocusListener, TableModelListener, MouseListener, ListSelectionListener {
    ...
    JCheckBox chkIGV;
    private JTextField txtPreimpresoFact;
    private JComboBox cboSerieFact;
    private List<UsuarioCorrelativo> x_seriecorrelativo;

    public UiFacturarServicioTransporte(PanelTFServicioTransporte pnltf, String title, Usuario usuario, Main frm) {
        super(title + " - RegisterServicioTransporte");
    ...
    private void inicialize() {
        ...
        JLabel lblSerieFact = new JLabel("Serie");
        lblSerieFact.setBounds(10, 5, 60, 20);
        pnlDialog.add(lblSerieFact);

        cboSerieFact = new JComboBox();
        cboSerieFact.addActionListener(this);
        cboSerieFact.setBounds(125, 5, 50, 20);
        cboSerieFact.addKeyListener(this);
        cboSerieFact.addActionListener(this);
        JTextField txt = new JTextField();
        cboSerieFact.setBorder(txt.getBorder());
        pnlDialog.add(cboSerieFact);


        JLabel lblPreimpresoFact = new JLabel("Preimpreso");
        lblPreimpresoFact.setBounds(200, 5, 60, 20);
        pnlDialog.add(lblPreimpresoFact);

        txtPreimpresoFact = new JTextField();
        txtPreimpresoFact.addActionListener(this);
        txtPreimpresoFact.setBounds(280, 5, 85, 20);
        txtPreimpresoFact.addKeyListener(this);
        txtPreimpresoFact.setFont(new Font(Font.SANS_SERIF, 0, 11));
        txtPreimpresoFact.addFocusListener(this);
        txtPreimpresoFact.setDocument(new IntegerDocument(10));
        txtPreimpresoFact.setForeground(Color.BLACK);
        txtPreimpresoFact.setEnabled(false);
        pnlDialog.add(txtPreimpresoFact);

        JLabel lblfecha = new JLabel("Fecha");
        lblfecha.setBounds(420, 5, 50, 20);
        pnlDialog.add(lblfecha);

        getDc_fechaemision();
        JLabel lblServicio = new JLabel("Servicio");
        lblServicio.setBounds(10, 35, 60, 20);
        pnlDialog.add(lblServicio);


        lbl_idmovinventario = new JLabel("Pto Origen");
        lbl_idmovinventario.setBounds(10, 95, 80, 20);
        pnlDialog.add(lbl_idmovinventario);

        lbl_tmpanexo = new JLabel("Cliente");
        lbl_tmpanexo.setBounds(10, 65, 50, 20);
        pnlDialog.add(lbl_tmpanexo);

        lbl_tmpruc = new JLabel("RUC/DNI");
        lbl_tmpruc.setBounds(420, 65, 50, 20);
        pnlDialog.add(lbl_tmpruc);

        cbo_idempresatransportista = new JComboBox();
        cbo_idempresatransportista.setBounds(125, 125, 250, 20);
        pnlDialog.add(cbo_idempresatransportista);

        lbl_idempresaTransportista = new JLabel("Emp. Transportista");
        lbl_idempresaTransportista.setBounds(10, 125, 100, 20);
...
//        }
    ...
    public void loadMoneda() {
        try {
            RnMoneda regla_Moneda = new RnMoneda(path);
            ...
            xmoneda.addAll(regla_Moneda.listarMon("", "A"));

            cbo_idmoneda.removeAllItems();
            cbo_idmoneda.addItem("--- Seleccione una Moneda ---");

            for (int i = 0; i < xmoneda.size(); i++) {
                cbo_idmoneda.addItem(xmoneda.get(i).getDescripcion());
            }
...
//            cab.setM_igv(Double.parseDouble(txt_igv.getText()));
//            cab.setMonto(Double.parseDouble(txt_total.getText()));
//            cab.setId_usuario(usuario.getId_usuario());
//            cab.setId_estado("003");
//            cab.setId_anexo_empresa_transportista(xEmpresaTransportista.get(cbo_idempresatransportista.getSelectedIndex() - 1).toString());
//            cab.setAnexo_transportista(cbo_idempresatransportista.getSelectedItem().toString());
...
//            mdl_servicio_transporte.clearTable();
    ...
    public boolean executeDelete() {
        try {
            ...
            DAOServicioTransporte dao = new DAOServicioTransporte(path);
            ArrayList<ContaItem> Citem = mdl_servicio_transporte.getData();
            ContaItem obj = Citem.get(0);
            String fecha = dc_fechaemision.getDate().toString();
            RegContaItem regItem = new RegContaItem();
            RegContaCab regCab = new RegContaCab();
            regCab.setId_tipo_doc("01");
            regCab.setSerie(cboSerieFact.getSelectedItem().toString());
            regCab.setPreimpreso(txtPreimpresoFact.getText());
            regCab.setM_monto(new BigDecimal(txt_total.getText()));
            regCab.setF_emision(dc_fechaemision.getDate());
            regCab.setId_auxiliar("00070");
            regCab.setEstado("05");
            regCab.setId_periodo_mensual(fecha.substring(0, 4));
            regCab.setId_anexo(txt_CodCliente.getText());
            regCab.setId_moneda(obj.getIdMoneda());
            regCab.setTipo_anexo("2");
            regCab.setTipo_operacion("I");
            regCab.setId_usuario(usuario.getId_usuario());
            regCab.setTmp_anexo(txt_tmpanexo.getText());
            regCab.setTmp_ruc(txt_tmpruc.getText());
            regCab.setTmp_direccion(txt_direccion_origen.getText());
            regItem.setId_item(xItemServicio.get(cboItemServicio.getSelectedIndex()).getId_item());
            regItem.setId_cuenta("70411");
//            regItem.setAlmacen(txt);
            regItem.setRegconta(regCab);
            String res = dao.facturar(regItem);
            return true;
        } catch (Exception ex) {
            ex.printStackTrace();
            cuadro.CuadroAviso(ex.toString(), JOptionPane.ERROR_MESSAGE);
            return false;
        } finally {
            try {
    ...
    public void showMessagePrint(String codigo) {
        try {
            ...
            report.generarReporte("Ingreso", codigo, "", "", "", "", "", true, false,
                    "Reporte Servicio de Transporte");
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, ex.toString());
        }
    }
    ...
    public String executeUpdate() {
        ...
        m.setMDescuento(Double.valueOf(txt_descuento.getText().trim()));
...
//            mdl_servicio_transporte.clearTable();
    ...
    public void loadSerieCorrelativo(String ls_IdTipoDoc) {
        try {
            RnCorrelativo regla_correlativo = new RnCorrelativo(path);

            if (x_seriecorrelativo != null) {
                x_seriecorrelativo.clear();
            } else {
                x_seriecorrelativo = new ArrayList<UsuarioCorrelativo>();
            }

//        x_seriecorrelativo.addAll(regla_correlativo.listarUsuarioCorrelativo("", usuario.getId_usuario(), "", "", usuario.getCodempresa(), ls_IdTipoDoc, usuario.getCodpuntoventa(), ""));
            x_seriecorrelativo.addAll(regla_correlativo.listarCorrelativo(usuario.getId_usuario(), usuario.getCodpuntoventa(), ls_IdTipoDoc));
            cboSerieFact.removeAllItems();

            for (int i = 0; i < x_seriecorrelativo.size(); i++) {
                cboSerieFact.addItem(x_seriecorrelativo.get(i).getSerie());
            }

            if (cboSerieFact.getItemCount() > 0) {
                cboSerieFact.setSelectedIndex(0);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
        }
    ...
    public void mostrarPreimpreso() {
        try {
            ...
            uc.setSerie(x_seriecorrelativo.get(cboSerieFact.getSelectedIndex()).getSerie());

            RnCorrelativo regla_correlativo = new RnCorrelativo(path);
            String preimpreso = regla_correlativo.listarPreimpreso(x_seriecorrelativo.get(cboSerieFact.getSelectedIndex()).getIdCorrelativo());
            txtPreimpresoFact.setText(Util.getPreimpresoValue(preimpreso));
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (cboSerieFact == e.getSource()) {
            if ((cboSerieFact.getSelectedIndex() > -1)) {
                mostrarPreimpreso();
            }
        }
    ...
    private static final Logger LOG = Logger.getLogger(UiFacturarServicioTransporte.class.getName());

    private JComboBox getCboItemServicio() {
        if (cboItemServicio == null) {
            cboItemServicio = new JComboBox();
            cboItemServicio.setBounds(125, 35, 240, 20);
    ...
    private JTextField getTxtDireccionOrigen() {
        if (txt_direccion_origen == null) {
            ...
            txt_direccion_origen.setBounds(125, 95, 190, 20);
    ...
    private JTextField getTxt_tmpanexo() {
        if (txt_tmpanexo == null) {
            ...
            txt_tmpanexo.setBounds(125, 65, 280, 20);
    ...
    private JTextField getTxt_tmpruc() {
        if (txt_tmpruc == null) {
            ...
            txt_tmpruc.setBounds(470, 65, 80, 20);
