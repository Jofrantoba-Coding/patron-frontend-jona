 ...
 */
package com.softcommerce.views.uipaneltfmotivonc;
...
import com.softcommerce.general.tablas.MotivoNotaCreditoTableModel;
import com.softcommerce.general.controles.PopupMenuView;
import java.awt.event.WindowListener;
import javax.swing.event.ListSelectionListener;
import com.softcommerce.general.controles.JHInternalFrame;
import com.softcommerce.formularios.RegisterMotivoNotaCredito;
 ...
 * @author TOSHIBA
 */
public class UiPanelTFMotivoNC extends JHInternalFrame implements InterUiPanelTFMotivoNC, ListSelectionListener, WindowListener, FocusListener, KeyListener, ActionListener, ItemListener {

    private static final long serialVersionUID = 1L;
    public CTable table;
    public MotivoNotaCreditoTableModel modeltable;
    public TableRowSorter<MotivoNotaCreditoTableModel> modeloOrdenado;
    ...
    public UiPanelTFMotivoNC(String title, JFrame frame, Usuario usuario) {
        super(title);
        this.usuario = usuario;
        this.frame = frame;
        inicialize();
    }
    public UiPanelTFMotivoNC(String title, JFrame frame, Usuario usuario, List<Boolean> vPermiso) {
    ...
    private void inicialize() {
        modeltable = new MotivoNotaCreditoTableModel();
        modeloOrdenado = new TableRowSorter<MotivoNotaCreditoTableModel>(modeltable);
        table = new CTable();
    ...
    public JPanel getPanelFilter() {
        ...
        txt_iditem.setBounds(80, 25, 90, 20);
        ...
        lblItem.setBounds(220, 25, 35, 20);
        pnlp.add(lblItem);

        txt_descripcionitem = new JTextField();
        txt_descripcionitem.setBounds(255, 25, 360, 20);
    ...
    public RowFilter<Object, Object> getFilter() {
        ...
        if (txt_iditem.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_iditem.getText().trim() + ".*", 1));
        }

        if (txt_descripcionitem.getText().trim().length() > 0) {
            filters.add(RowFilter.regexFilter(".*" + txt_descripcionitem.getText().trim() + ".*", 2));
        }

        if (cbo_idmarca.getSelectedIndex() > 0) {
            filters.add(RowFilter.regexFilter(".*" + cbo_idmarca.getSelectedItem() + ".*", 3));
        }
    ...
    public void loadMarca() {
        cbo_idmarca.addItem("T");
        cbo_idmarca.addItem("NOTA CREDITO COMPRA");
        cbo_idmarca.addItem("NOTA CREDITO VENTA");
        cbo_idmarca.addItem("NOTA DEBITO VENTA");
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == cbo_idmarca) {
            if (cbo_idmarca.getItemCount() > 0) {
                if (cbo_idmarca.getSelectedIndex() >= 0) {
                    filtrar();
                }
            }
    ...
    public void controlAdd() {
        ...
        RegisterMotivoNotaCredito register = new RegisterMotivoNotaCredito(frame, usuario);
    ...
    public void focusLost(FocusEvent e) {
        if (e.getSource() == txt_iditem && txt_iditem.getText().trim().length() > 0) {
            String serie = "000" + txt_iditem.getText().trim();
