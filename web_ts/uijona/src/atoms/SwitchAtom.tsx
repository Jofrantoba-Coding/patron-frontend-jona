...
export const SwitchAtom: React.FC<SwitchAtomProps> = ({
  ...
  disabled = false, hasError = false, size = 'md', className,
  ...
  return (
    <button
      ...
      aria-invalid={hasError || undefined} aria-label={ariaLabel} aria-labelledby={ariaLabelledBy}
      disabled={disabled}
      ...
      className={cn(
        ...
        checked ? (hasError ? 'bg-danger-500' : 'bg-primary-600') : 'bg-neutral-300',
        track, className
      )}
    >
      <span aria-hidden="true" className={cn('pointer-events-none inline-block rounded-full bg-white shadow-sm transition-transform duration-200', checked ? translate : 'translate-x-0', thumb)} />
    </button>
  );
};
