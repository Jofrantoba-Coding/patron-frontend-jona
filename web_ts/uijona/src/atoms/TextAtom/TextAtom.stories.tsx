...
export const AllColors: Story = {
  render: () => (
    <JPanel variant="ghost" padding="none" style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
...
export const Interactive: Story = {
  render: () => {
    ...
    return (
      <JPanel variant="ghost" padding="none" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <TextAtom color={colorMap[status]}>{msgMap[status]}</TextAtom>
        <JPanel variant="ghost" padding="none" style={{ display: 'flex', gap: '8px' }}>
          <JButton variant="outline" size="sm" onClick={() => setStatus('success')}>Éxito</JButton>
          <JButton variant="outline" size="sm" onClick={() => setStatus('error')}>Error</JButton>
          <JButton variant="outline" size="sm" onClick={() => setStatus('idle')}>Reiniciar</JButton>
        </JPanel>
      </JPanel>
    );
  },
