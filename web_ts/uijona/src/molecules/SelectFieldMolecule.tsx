...
export const SelectFieldMolecule = React.forwardRef<HTMLSelectElement, SelectFieldMoleculeProps>(
  ({ id, label, options, groups, placeholder, errorMessage, description, required, className, ...selectProps }, ref) => {
    const hasError = !!errorMessage;
    return (
      <div className={cn('flex flex-col gap-1.5', className)}>
        <LabelAtom htmlFor={id} required={required}>{label}</LabelAtom>
        <SelectAtom ref={ref} id={id} options={options} groups={groups} placeholder={placeholder} hasError={hasError} required={required} aria-describedby={description ? `${id}-desc` : undefined} {...selectProps} />
