...
export const EmptyStateMoleculeView: React.FC<InterEmptyStateMolecule> = ({
  ...
  >
    {icon && (
      <PanelAtom variant="ghost" padding="none" radius="none" className="flex h-12 w-12 items-center justify-center rounded-full bg-neutral-100 text-neutral-500">
        {icon}
      </PanelAtom>
    )}
    <PanelAtom variant="ghost" padding="none" radius="none" className="flex max-w-md min-w-0 flex-col gap-1">
    ...
    {actions && actions.length > 0 && (
      <PanelAtom variant="ghost" padding="none" radius="none" className="flex flex-wrap items-center justify-center gap-2">
        {actions.map((action) => (
          <ButtonAtom
            key={action.label}
            type="button"
