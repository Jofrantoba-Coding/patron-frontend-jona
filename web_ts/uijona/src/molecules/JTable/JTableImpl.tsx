...
import { TableContext, useTableContext } from './JTableContext';
import {
  JTableView,
  TableCaptionView,
  TableHeaderView,
...
function isColumnGroup(col: JTableColumnDef | JTableColumnGroup): col is JTableColumnGroup {
  return 'columns' in col && Array.isArray((col as JTableColumnGroup).columns);
}

function getLeafColumns(columns: JTableColumnsConfig): JTableColumnDef[] {
  if (columns.length === 0) return [];
  return isColumnGroup(columns[0] as JTableColumnDef | JTableColumnGroup)
    ? (columns as JTableColumnGroup[]).flatMap((g) => g.columns)
    : (columns as JTableColumnDef[]);
}

function sortDataByKey(
  data: Record<string, unknown>[],
  key: string,
  direction: JTableSortDirection
...
export const JTableImpl = forwardRef<HTMLTableElement, InterJTable>(
  ({
    children,
...
TableHeaderImpl.displayName = 'JTableHeader';
...
TableBodyImpl.displayName = 'JTableBody';
...
function DataDrivenTableContent({
  ...
  const [internalSort, setInternalSort] = useState<{ key: string; direction: JTableSortDirection }>({
    key: '',
    direction: null,
  });

  const hasGroups = columns.length > 0 && isColumnGroup(columns[0] as JTableColumnDef | JTableColumnGroup);
  ...
  const handleSortChange = (col: JTableColumnDef) => (direction: JTableSortDirection) => {
    if (col.onSortChange) {
  ...
  const renderHeadCell = (col: JTableColumnDef, colIdx: number) => {
    ...
    <>
      ...
      <TableHeaderView>
        {hasGroups ? (
          <>
            <TableRowView>
              {(columns as JTableColumnGroup[]).map((group) => (
          ...
          <TableRowView>
            {(columns as JTableColumnDef[]).map((col, colIdx) => renderHeadCell(col, colIdx))}
          </TableRowView>
        )}
      </TableHeaderView>
      <TableBodyView>
