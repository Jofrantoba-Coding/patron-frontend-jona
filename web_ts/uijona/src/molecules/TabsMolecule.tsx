...
const TabsContext = createContext<TabsContextValue>({ value: '', onChange: () => {}, variant: 'pill', orientation: 'horizontal' });
...
export const TabsMolecule: React.FC<TabsMoleculeProps> = ({ value, onChange, onTabFocus, onDisabledTabClick, variant = 'pill', orientation = 'horizontal', className, children }) => (
...
export const TabsList: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, children, ...props }) => {
  ...
  return (
    <div role="tablist" aria-orientation={orientation} className={cn('flex', orientation === 'vertical' ? 'flex-col' : 'flex-row', variant === 'pill' ? 'bg-neutral-100 rounded-md p-1 gap-1' : 'border-b border-neutral-200 gap-0', className)} {...props}>{children}</div>
  );
};

interface TabsTriggerProps extends React.ButtonHTMLAttributes<HTMLButtonElement> { value: string; }

export const TabsTrigger: React.FC<TabsTriggerProps> = ({ value, className, children, disabled, ...props }) => {
  ...
  return (
    <button role="tab" type="button" aria-selected={isActive} disabled={disabled}
      ...
      className={cn('inline-flex items-center justify-center gap-1.5 text-sm font-medium transition-all duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 disabled:pointer-events-none disabled:opacity-50',
        variant === 'pill' ? cn('px-3 py-1.5 rounded', isActive ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700')
