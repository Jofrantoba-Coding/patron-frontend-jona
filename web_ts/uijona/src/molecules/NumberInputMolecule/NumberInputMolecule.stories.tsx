  ...
  render: (args) => {
    ...
    return (
      <PanelAtom variant="ghost" padding="none" className="flex flex-col gap-3 w-56">
        <TextAtom size="sm" className="font-medium">Cantidad en carrito</TextAtom>
        <NumberInputMolecule
          min={1}
          max={10}
          step={1}
        ...
        <PanelAtom variant="ghost" padding="none" className="rounded-md bg-neutral-50 border p-3 text-sm">
          <TextAtom className="text-neutral-500">Precio unitario: <strong>${pricePerUnit.toFixed(2)}</strong></TextAtom>
          <TextAtom className="text-neutral-800 font-semibold mt-1">Total: ${total.toFixed(2)}</TextAtom>
        </PanelAtom>
      </PanelAtom>
    );
  },
