...
export const FormFieldMolecule = React.forwardRef<HTMLInputElement, FormFieldMoleculeProps>(
  ({ id, label, errorMessage, description, orientation = 'vertical', required, className, onChange, onBlur, onFocus, onKeyDown, onEnterPress, onClear, onValid, onInvalid, ...inputProps }, ref) => {
    ...
    return (
      <div className={cn(orientation === 'horizontal' ? 'flex items-center gap-4' : 'flex flex-col gap-1.5', className)}>
        ...
        <div className={cn('flex flex-col gap-1', orientation === 'horizontal' && 'flex-1')}>
          <InputAtom ref={ref} id={id} hasError={hasError} required={required} aria-describedby={description ? `${id}-desc` : undefined} onChange={onChange} onBlur={handleBlur} onFocus={onFocus} onKeyDown={onKeyDown} onEnterPress={onEnterPress} onClear={onClear} {...inputProps} />
