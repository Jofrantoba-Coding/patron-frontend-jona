...
export const DialogMoleculeView: React.FC<DialogMoleculeViewProps> = ({
  ...
  if (!open) return null;
  return createPortal(
    <JPanel variant="ghost" padding="none" radius="none" ref={overlayRef} role="presentation" className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:items-center">
      <JPanel variant="ghost" padding="none" radius="none" className="absolute inset-0 bg-black/50" aria-hidden="true" onClick={onOverlayClick} />
      ...
      >
        {(title || showCloseButton) && (
          <JPanel variant="ghost" padding="none" radius="none" className="flex items-start justify-between gap-4">
            ...
            {showCloseButton && (
              <JButton variant="ghost" size="icon" onClick={onCloseClick} aria-label="Close dialog" className="shrink-0 -mt-1 -mr-1">
        ...
        {children && <JPanel variant="ghost" padding="none" radius="none" className="min-w-0 text-sm text-neutral-700">{children}</JPanel>}
        {footer && <JPanel variant="ghost" padding="none" radius="none" className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">{footer}</JPanel>}
      </JPanel>
    </JPanel>,
    document.body
  );
