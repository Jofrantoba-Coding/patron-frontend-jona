...
export const DialogMoleculeView: React.FC<DialogMoleculeViewProps> = ({
  ...
  if (!open) return null;
  return createPortal(
    <div ref={overlayRef} role="presentation" className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:items-center">
      ...
      >
        ...
        {children && <div className="min-w-0 text-sm text-neutral-700">{children}</div>}
        {footer && <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">{footer}</div>}
      </div>
    </div>,
    document.body
  );
