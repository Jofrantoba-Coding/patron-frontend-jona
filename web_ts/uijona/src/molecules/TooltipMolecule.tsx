...
export const TooltipMolecule: React.FC<TooltipMoleculeProps> = ({ content, side = 'top', delayMs = 300, className, children, onShow, onHide }) => {
  ...
  const show = () => { timerRef.current = setTimeout(() => { setStyle(computeStyle()); setVisible(true); onShow?.(); }, delayMs); };
  const hide = () => { if (timerRef.current) clearTimeout(timerRef.current); setVisible(false); onHide?.(); };

  return (
    <>
      <span ref={triggerRef} className="inline-flex" onMouseEnter={show} onMouseLeave={hide} onFocus={show} onBlur={hide}>{children}</span>
      {visible && createPortal(
        <div role="tooltip" style={style} className={cn('max-w-xs rounded bg-neutral-900 px-2.5 py-1.5 text-xs text-white shadow-md pointer-events-none select-none', className)}>{content}</div>,
        document.body
      )}
    </>
  );
