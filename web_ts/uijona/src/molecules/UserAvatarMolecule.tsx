...
export const UserAvatarMolecule: React.FC<UserAvatarMoleculeProps> = ({ name, email, size = 'md', className }) => {
  const initials = name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2);
  return (
    <div className={cn('flex items-center gap-3', className)}>
      <div className={cn('rounded-full bg-primary-600 flex items-center justify-center text-white font-semibold flex-shrink-0', sizeClasses[size].avatar)} aria-label={`Avatar of ${name}`}>
        {initials || '?'}
      </div>
