// JTimerView.tsx — JONA View (render puro)
...
const variantClasses: Record<JTimerVariant, string> = {
  plain:  'bg-transparent',
  card:   'rounded-lg border border-neutral-200 bg-white p-4 shadow-sm sm:p-5',
  inline: 'inline-flex items-center gap-3',
};

const sizeClasses: Record<JTimerSize, { display: string; label: string; button: string }> = {
...
const toneClasses: Record<JTimerTone, string> = {
...
const statusText: Record<JTimerStatus, string> = {
  idle:      'Idle',
  running:   'Running',
  paused:    'Paused',
  completed: 'Completed',
...
export const JTimerView: React.FC<JTimerViewProps> = ({
  status,
  context,
  displayValue,
  canStart,
  ...
  return (
    <JPanel
      ...
      padding="none"
      radius="none"
      className={cn('min-w-0', variantClasses[variant], !isInline && 'flex flex-col gap-3', className)}
      data-status={status}
    ...
    >
      {(label || completedLabel) && !isInline && (
        <JPanel variant="ghost" padding="none" radius="none" className="min-w-0">
          {label && (
            <p className={cn('break-words font-medium text-neutral-500', sizeClasses[size].label)}>
              {label}
            </p>
          )}
          {status === 'completed' && completedLabel && (
            <p className={cn('mt-1 break-words font-medium', sizeClasses[size].label, toneClasses[tone])}>
              {completedLabel}
            </p>
          )}
        </JPanel>
      ...
      <JPanel variant="ghost" padding="none" radius="none" className={cn('min-w-0', isInline && 'flex items-center gap-3')}>
        ...
        {controls && (
          <JPanel
            ...
            padding="none"
            radius="none"
            className={cn('flex flex-wrap items-center gap-2', isInline ? 'ml-1' : 'mt-3')}
          >
            {canStart && (
              <button
                type="button"
