...
export const SwitchFieldMolecule: React.FC<SwitchFieldMoleculeProps> = ({
  ...
  if (card) {
    return (
      <div className={cn('rounded-md border border-neutral-200 p-4 cursor-pointer hover:bg-neutral-50 transition-colors duration-150', disabled && 'pointer-events-none opacity-50', className)} onClick={() => !disabled && onCheckedChange?.(!checked)}>
        {inner}
      </div>
    );
  }
