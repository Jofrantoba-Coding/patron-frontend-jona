import type { Meta, StoryObj } from '@storybook/react';
import { useState } from 'react';
import { JFooterPage } from './JFooterPage';
import { JButton } from '../../atoms/JButton/JButton';
import { JPanel } from '../../atoms/JPanel/JPanel';
import { JLabel } from '../../atoms/JLabel';

const meta: Meta<typeof JFooterPage> = {
  title: 'Organisms/JFooterPage',
  component: JFooterPage,
  tags: ['autodocs'],
  parameters: { layout: 'fullscreen' },
};
export default meta;
type Story = StoryObj<typeof JFooterPage>;

export const Default: Story = {
  args: { text: '© 2026 JONA UI — Todos los derechos reservados' },
};

export const WithSlots: Story = {
  args: {
    left:   <JLabel as="span" className="font-bold">JONA UI</JLabel>,
    center: (
      <nav style={{ display: 'flex', gap: '16px' }}>
        <a href="#">Términos</a>
        <a href="#">Privacidad</a>
        <a href="#">Contacto</a>
      </nav>
    ),
    right: <JLabel as="span">v1.2.5</JLabel>,
  },
};

export const Interactive: Story = {
  render: () => {
    const [lang, setLang] = useState<'es' | 'en'>('es');
    const texts = {
      es: { terms: 'Términos', privacy: 'Privacidad', contact: 'Contacto' },
      en: { terms: 'Terms', privacy: 'Privacy', contact: 'Contact' },
    };
    const t = texts[lang];
    return (
      <JPanel variant="ghost" padding="none" style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
        <JPanel variant="ghost" padding="none" style={{ padding: '32px', textAlign: 'center', fontSize: '14px', color: '#a3a3a3' }}>
          Contenido de la aplicación
        </JPanel>
        <JFooterPage
          left={<JLabel as="span" className="font-bold">JONA UI</JLabel>}
          center={
            <nav style={{ display: 'flex', gap: '16px', fontSize: '14px' }}>
              <a href="#">{t.terms}</a>
              <a href="#">{t.privacy}</a>
              <a href="#">{t.contact}</a>
            </nav>
          }
          right={
            <JButton variant="outline" size="sm" onClick={() => setLang((l) => l === 'es' ? 'en' : 'es')}>
              {lang === 'es' ? 'EN' : 'ES'}
            </JButton>
          }
        />
      </JPanel>
    );
  },
};
