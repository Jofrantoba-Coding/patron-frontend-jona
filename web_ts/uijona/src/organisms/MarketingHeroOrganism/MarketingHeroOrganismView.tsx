...
const ctaVariant: Record<NonNullable<MarketingHeroCTA['variant']>, string> = {
...
export const MarketingHeroOrganismView: React.FC<InterMarketingHeroOrganism> = ({
  ...
  <section
    className={cn('relative overflow-hidden bg-neutral-900 py-20 sm:py-28', className)}
    {...props}
  >
    <SectionShellAtom>
      <div className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
        <div className="flex flex-col gap-6">
          ...
          {subtitle && (
            <p className="text-lg leading-relaxed text-white/70 sm:text-xl">{subtitle}</p>
          )}
          {ctas && ctas.length > 0 && (
            <div className="flex flex-wrap gap-3 pt-2">
              {ctas.map((cta) =>
                cta.href ? (
                  <a key={cta.label} href={cta.href} className={cn(ctaBase, ctaVariant[cta.variant ?? 'primary'])}>
                    {cta.label}
                  </a>
                ) : (
                  <button key={cta.label} type="button" onClick={cta.onClick} className={cn(ctaBase, ctaVariant[cta.variant ?? 'primary'])}>
                    {cta.label}
                  </button>
                )
              )}
        ...
        {visual && (
          <div className="flex items-center justify-center lg:justify-end">{visual}</div>
        )}
      </div>
    </SectionShellAtom>
  </section>
